...
- Tests: `src/lib/nostr/__tests__/viewerKey.test.ts` (determinism; domain separation by version/pubkey/sk — the
  ...
  settings + plan backup, per-tier build). SETTINGS_FIELDS count **37** after Multi-viewer M1 (−5 single-viewer
  scalars +2 roster fields; the doc's earlier "39 → 36" arithmetic was wrong — 37 is the pinned reality, see
  slices.test.ts / viewerSnapshot.test.ts).
