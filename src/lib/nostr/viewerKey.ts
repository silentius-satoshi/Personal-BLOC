...
// in person. Because it is deterministic in (ownerSk, ownerPubkeyHex, version), re-running "Generate viewer
// key" yields the identical nsec — that IS the recovery/regeneration property.
//
// CRYPTO DOMAIN: this is its OWN domain, separate from keyVault.ts. It uses WebCrypto directly and does NOT
// import keyVault's helpers or reuse its HKDF info labels — a distinct `personal-bloc/viewer-key/v${version}`
...
/**
 * Derive a 32-byte viewer secret key from the owner's secret key.
 *
 * HKDF-SHA256: ikm = owner sk, salt = SHA-256(utf8(ownerPubkeyHex)), info =
 * `personal-bloc/viewer-key/v${version}`, deriveBits 256.
 *
...
export async function deriveViewerKeyFromNsec(
  ...
  const baseInfo = `personal-bloc/viewer-key/v${version}`;
