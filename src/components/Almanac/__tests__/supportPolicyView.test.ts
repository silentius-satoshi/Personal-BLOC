...
import {
  ...
  effectivePolicySettings, ZONE_LABEL, ZONE_LETTER, policyReading, policyHeadline, policyDetails, neverDrawsNote,
  policyPauseReason, drawPauseClause, zoneStrip, policyLimitPct, coldShown, strikeCallSummary, strikeCallSentence,
...
describe('effectivePolicySettings — the clamp, the push, and the stops the run USES', () => {
  ...
  it('the defaults pass through unclamped', () => {
    ...
    expect(s).toMatchObject({ ...DEFAULT_SUPPORT_POLICY_SETTINGS, cbClamped: false, skClamped: false });
...
describe('policyHeadline — ONE line, the bad tone first', () => {
  ...
  const ORDER: [string, Partial<PolicyReading>, PolicyTone, string][] = [
    ['latched break', LATCHED, 'bad', `${BROKEN_HEAD} It stays that way for the rest of this run.`],
...
describe('policyReading on Run 1\'s own fixtures', () => {
  ...
  it('⭐ P6: a latched break reads bad; the same break re-armed at 6 reads warn (v1.1 #2)', () => {
    ...
    expect(policyHeadline(read(latched, 20), S)).toEqual({ tone: 'bad', text: `${BROKEN_HEAD} It stays that way for the rest of this run.` });
...
describe('policyPauseReason — the real reason a month did not borrow', () => {
  ...
  it('one sentence per zone — the spec\'s words', () => {
    expect(policyPauseReason(hold, S)).toBe('Price is 1.62× support — above the 1.5× buy zone, so no new debt: your paycheck pays the bills.');
    expect(policyPauseReason({ ...payDown, payDownUsd: 2_000 }, S))
      .toBe('Price is 2.31× support — above 2×, so spare income pays debt down ($2,000 this month) before buying.');
    expect(policyPauseReason(paused, S)).toBe('Price is under support (0.94×) — the policy borrows nothing until it is back on the line.');
    expect(policyPauseReason(broken, S)).toBe('The model is treated as broken — no new debt.');
    // The row cannot say whether a limit or Strike's own line bound, so the sentence names both.
    expect(policyPauseReason({ ...blocked, restoreUsd: 0 }, S))
      .toBe("The limits at support (or Strike's own line) leave no room to borrow this month, so your paycheck pays the bills.");
  });

  it('pay down with nothing paid: "none this month" with debt left, "no debt left" without — never "($0 this month)"', () => {
    expect(payDown.debt).toBeGreaterThan(0);
    expect(policyPauseReason({ ...payDown, payDownUsd: 0 }, S))
      .toBe('Price is 2.31× support — above 2×, so spare income pays debt down before buying (there was none this month).');
    expect(policyPauseReason({ ...payDown, payDownUsd: 0, debt: 0 }, S))
  ...
  it('the restore / cash / unpaid suffixes, in that order — and the paycheck "pays what it can" when it did not cover', () => {
    expect(policyPauseReason({ ...blocked, restoreUsd: 1_200 }, S))
    ...
    expect(policyPauseReason({ ...hold, cashToBillsUsd: 2_000 }, S))
    ...
    expect(policyPauseReason({ ...hold, unfundedUsd: 2_000 }, S))
    ...
    expect(policyPauseReason({ ...blocked, restoreUsd: 1_200, cashToBillsUsd: 2_000, unfundedUsd: 1_000 }, S))
    ...
    expect(policyPauseReason({ ...hold, restoreUsd: 1e-10 }, S)).toBe(policyPauseReason(hold, S));
  });
  ...
  it('null on a drawing row, with the policy off, and at month 0; the loop\'s end after a liquidation', () => {
    ...
    expect(policyPauseReason(drew, S)).toBeNull();
    expect(policyPauseReason(runCyclingSim({ ...SP_REPRO, pricePath: pathP2(0) }).rows[20], S)).toBeNull();
    expect(policyPauseReason(p2.rows[0], S)).toBeNull();
    ...
    expect(policyPauseReason(liq.rows[1], S)).toBe(
