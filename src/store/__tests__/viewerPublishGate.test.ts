...
// Read-only-viewer backstop: publishRecordsNow must NEVER publish in viewerMode (a viewer is authenticated with its
...
describe('publishRecordsNow — viewerMode gate', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  ...
  it('Case A — a read-only viewer is BLOCKED (returns false at the gate, never reaches the publish attempt)', async () => {
    ...
    const result = await publishRecordsNow();
