...
**Current shipped baseline: store **v15** · 215/215 tests · HEAD `60ac02f` (branch `main`).**
**Last rebuilt:** after the **security arc** (writer local-nsec signer → owner-pubkey gate → NIP-98 proxy auth) + the position-box relayout arc. Historical record of what shipped; the OPEN tail is the live queue (deploy/iOS verification + the queued viewer-access spec).
...
| **Vercel env changes (security arc)** | deploy | Set **before/with** the deploy or the gate breaks: **REMOVE** `VITE_APP_PROXY_SECRET` + `APP_PROXY_SECRET`; **ADD** `OWNER_PUBKEY` (server hex, = `VITE_OWNER_PUBKEY`) + `PUBLIC_ORIGIN` (server, exact deploy origin — `window.location.origin`, no trailing slash). `STRIKE_API_KEY` stays (mark sensitive); pubkeys need not be sensitive. ✅ user confirmed set. |
| **iOS local signer (Step 4)** | deploy/device | "Use a local key" appears (iOS-only); backup gate blocks until checked; non-nsec rejected; Face ID enrolls (or PIN); logs in, pubkey matches; **full reload → unlock screen (not full login)**; NIP-07/46 relaunch unchanged (silent); publish works after unlock. PRF needs iOS Safari 18.4+ — not jsdom-testable. |
| **Owner-gate** | deploy | Un-authenticated visitor → auth gate, **NO `/api/strike-balances` in the network tab** (the closed leak); owner → full app + Strike; foreign nsec → "private app" notice; `VITE_OWNER_PUBKEY` unset → fallback (any auth key); DEV bypasses. |
| **NIP-98 proxy** | deploy | Owner authenticated → balances render (on NIP-46, token-cache = remote-sign ~every 50s, no prompt storm); `curl` no/bad token → **401**; different key's token → **403**; **`PUBLIC_ORIGIN` exactly matches deploy origin** (mismatch → 401 every fetch — first-deploy gotcha); grep `dist/` → no proxy secret. |
...
| **Viewer-access spec (dad read-only)** | queued (MAX) | Phases 1–3 at `/mnt/project/personal-bloc-viewer-access-spec-v1.md`. **Two amendments from the security arc to fold in when built:** (a) the owner-gate's non-owner branch becomes `isAuthenticated && !isOwner && !viewerMode` (a provisioned viewer passes into read-only render); (b) the `ViewerSnapshot` + `publishViewerSnapshotNow` carry `strikeUsdBalance`/`strikeBtcAvailable` and the viewer hydrate path renders them read-only (**Option B** — dad sees live Strike via the encrypted snapshot, NEVER fetches the proxy; the owner-only fetch gate + NIP-98 403-for-non-owner is correct as-is). **Reuses the now-shipped `keyVault.ts`.** Phases renumber **15→16→17** (the writer local signer took the 14→15 slot the spec assumed). The natural next feature. |
