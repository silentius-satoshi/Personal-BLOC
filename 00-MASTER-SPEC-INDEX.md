...
| `viewer-access-spec-v2` Phase 2 | `d67bd8d` | **🔭 Read-only viewer client.** A fresh install can become a read-only viewer: first-run "View someone else's plan" fork generates its OWN disposable key (`generateSecretKey`), follows the owner's npub, fetches+decrypts the `viewer:v1` snapshot (`viewerSync.ts`, new — mirrors `liveSync`; `new NSecSigner(hexToBytes(viewerSecretKey).slice())` — the signer-copy lesson), and renders read-only with live updates (incl. Strike from the snapshot). Owner-gate amended: all three auth branches get `&& !viewerMode` so a viewer short-circuits to the app render. **Writer sync OFF by construction in viewerMode** (`useNostrSync({ live: !viewerMode })` + effect early-return + `triggerSync` no-op — the load-bearing read-only guarantee, not relying on null pubkey). Read-only = structural (no publish reachable) + UX (shared `NumberInput`/`Toggle`/`SliderInput`/pills gated on `viewerMode`). `useStrikeData(false)` for the viewer (Option B). Store **v16→v17** (device-local `viewerMode`/`viewerWriterPubkey`/`viewerSecretKey`, plaintext until Phase 3). |
| `copy-my-npub` button | `61cd502` | Settings NOSTR IDENTITY row gains a "Copy npub" button (`nip19.npubEncode(nostrPubkey)`, reuses the OnboardingModal "Copied ✓" pattern) — so the owner can hand their npub to a viewer. Both handshake directions now have a copy button. |
| viewer publish-on-save + DevPanel diagnostics | `70111f5` | **UX fix:** saving a viewer npub now fires `publishViewerSnapshotNow()` immediately (was: only on a settings/records *change*, so the viewer saw nothing until the owner edited — confusing). **DevPanel VIEWER ACCESS section:** handshake-state grid (role, publishes-to/reads-from pubkeys 8…8, viewer-key-present boolean, my-pubkey) + a **"Test viewer link"** probe that querySyncs the relays for the `viewer:v1` event and reports where the chain breaks (no event / decrypt-fails / OK + entry counts). Privacy rule: metadata/booleans/counts/ages/truncated-pubkeys only. `hexToBytes` via `nostr-tools/utils` (NOT `@noble/hashes/utils` — not in the exports map). |
| `viewer-access-spec-v2` Phase 3 | `17e24ac` | **🔭 Passkey-gated viewer key at rest.** The viewer's nsec was plaintext hex; now **wrapped at rest** (the shipped `keyVault.ts` — AES-GCM, Face-ID-PRF / PIN), unwrapped only after a Face ID/PIN gesture — mirroring the writer's `NostrAuthGate`-wraps / `LocalUnlockGate`-unwraps pattern. The unwrapped bytes live ONLY in an in-memory holder in `viewerSync` (`unwrappedViewerKey` + `setUnwrappedViewerKey`, mirroring a transient `viewerUnlocked` store boolean — never serializable state); `getViewerSigner()` builds from the holder with `.slice()`. New `ViewerUnlockGate.tsx` (unlock + migrant-setup modes). Onboarding wraps the generated key (no plaintext). **Back-compat migration 17→18:** a v17 plaintext-key viewer keeps syncing (back-fill the holder from the surviving plaintext) and is routed to a one-time wrap-setup that clears the plaintext. Recovery = re-provision a fresh key + new npub (lossless — all data is the owner's snapshot). Store **v17→v18**. Device-local `viewerKeyWrapped`/`viewerKeyWrapMeta`. |
| `passkey-user-label` | `cef1ab0` | Passkeys registered under the hardcoded `user.name: 'local-key'` were indistinguishable in the Face ID picker / iOS Settings → Passwords. Now the user **names** the key at registration: `wrapSecretKey(…, label?)` threads an optional label → `prfRegister` → the passkey username (fallback "Personal ₿LOC key"). PRF-only (the PIN scheme has no passkey — field hidden when capability is `pin`). Label input on the writer local-key + viewer onboarding PRF paths. **Not retroactive** — only keys registered after this are named (re-provision to rename existing). No store change. |
| viewer-config cross-device sync | `6673a2b` | **Fix:** `viewerNpub`/`viewerPubkey` (the owner's sharing config) were device-local — so a viewer set on desktop was invisible/un-removable on iOS, AND editing on iOS published no snapshot (no `viewerPubkey` to seal to). Now they **sync in the owner's own `settings:v1`** (public npubs, NIP-44 to self — no secret leak) added to `SETTINGS_FIELDS` + `buildSettingsPayload` + syncing setters. **CRITICAL boundary:** **stripped from the viewer's snapshot** (`buildViewerSnapshotPayload` rest-omits both fields) so the viewer never learns who else the owner shares with — pinned by an inverted boundary test. Viewer-SIDE fields (`viewerMode`/`viewerWriterPubkey`/`viewerSecretKey`/`viewerKeyWrapped`) stay device-local. No version bump (additive; migrate defaults already present). 220 tests (+1 strip test). |
...
| **Viewer follow-ups (minor, deferred)** | optional | The viewer arc (Phases 1–3) is COMPLETE and shipped. Two minor, non-blocking follow-ups remain: (a) **DevPanel probe → in-memory holder** — `runViewerProbe`'s viewer-side decrypt reads the plaintext `viewerSecretKey`, so for a *wrapped* (Phase-3) viewer it reports "no viewer key" rather than decrypt-verifying; the event-presence query still works and the owner/v17-migrant sides decrypt fine. Wire the probe to `viewerSync`'s holder to decrypt-verify a wrapped viewer. (b) **Passkey rename is not retroactive** — existing writer/viewer passkeys keep "local-key" until re-provisioned; optionally re-provision dad's viewer to give it a named passkey. Neither blocks anything. |
